...
```bash
curl -s -X POST "https://www.aicoo.io/api/v1/notes/42/snapshots" \
  -H "Authorization: Bearer $PULSE_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "https://www.aicoo.io/api/v1/notes/42/snapshots?limit=10" \
...
```bash
curl -s "https://www.aicoo.io/api/v1/notes/42/snapshots/7" \
  -H "Authorization: Bearer $PULSE_API_KEY" | jq .
```
...
```bash
curl -s -X POST "https://www.aicoo.io/api/v1/notes/42/snapshots/7/restore" \
  -H "Authorization: Bearer $PULSE_API_KEY" | jq .
```
